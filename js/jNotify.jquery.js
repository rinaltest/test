...
(function($){
	...
	$.jNotify = {
		defaults: {
			...
			InsideDiv : null,				// ability to choose the place where notification will be displayed
		...
		init:function(msg, options, id) {
			...
			WidthDoc = opts.InsideDiv ? parseInt($(opts.InsideDiv).width()) : parseInt($(window).width());
			HeightDoc = opts.InsideDiv ? parseInt($(opts.InsideDiv).height()) : parseInt($(window).height());

			// Scroll Position
			ScrollTop = opts.InsideDiv ? parseInt($(opts.InsideDiv).scrollTop()) : parseInt($(window).scrollTop());
			ScrollLeft = opts.InsideDiv ? parseInt($(opts.InsideDiv).scrollLeft()) : parseInt($(window).scrollLeft());
		...
		_construct:function(id, msg) {
			...
			.html(msg)
			.appendTo(opts.InsideDiv ? $(opts.InsideDiv) :'body');
			return $Div;
		},
		...
		_showOverlay:function(el){
			...
			if(opts.InsideDiv){
				$('#jOverlay').css({
					width: WidthDoc+'px',
					height: HeightDoc+'px',
					left: opts.InsideDiv.offset().left,
					top: opts.InsideDiv.offset().top
				});
			}
